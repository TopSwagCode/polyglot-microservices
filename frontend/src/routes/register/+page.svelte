...
<div class="auth-wrapper center" style="min-height:60vh;">
  <form class="surface register-form" on:submit|preventDefault={register}>
    ...
    <div class="field full">
      ...
      <input id="username" name="username" bind:value={username} required class="full" autocomplete="username" />
    </div>
    <div class="field full">
      <label for="password">Password</label>
      <input id="password" type="password" name="password" bind:value={password} required class="full" autocomplete="new-password" />
      <small class="hint">Minimum 8 characters.</small>
    </div>
    <div class="field full">
      <label for="confirm">Confirm Password</label>
      <input id="confirm" type="password" name="confirm" bind:value={confirm} required class="full" autocomplete="new-password" />
    </div>
    {#if error}
